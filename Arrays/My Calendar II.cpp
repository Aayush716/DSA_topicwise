    ...
    bool book(int start, int end) {
        ...
        for(pair<int, int> region : doubleOverlappedRegion) {
            if(checkOverlap(region.first, region.second, start, end)) {
                return false; //tripple booking created hence return false
            }
        }
        ...
        for(pair<int, int> booking : overallBookings) {
            if(checkOverlap(booking.first, booking.second, start, end)) {
                doubleOverlappedRegion.push_back(findOverlappedRegion(booking.first, booking.second, start, end));//now this is double overlapped hence put in that vector
            }
        }

        overallBookings.push_back({start, end});//since that was pushed in double overlaped it will be in single also
        return true;
    }
};
